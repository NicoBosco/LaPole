...
import { StandingsResponse, ResultsTableResponse, ConstructorTableResponse } from "@/types/api";
import {
  CURRENT_SEASON,
  REVALIDATE_DRIVERS,
...
export async function getConstructorCurrentStanding(
  ...
  const standing = res.MRData.StandingsTable.StandingsLists?.[0]?.ConstructorStandings?.[0];
