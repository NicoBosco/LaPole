...
export function ComparePageClient({ standings, season }: ComparePageClientProps) {
          ...
          <div className="overflow-x-auto rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)]">
            <div className="min-w-[380px]">
              ...
              <div className="grid grid-cols-3 border-b border-[var(--color-border)] bg-[var(--color-surface-raised)] px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">
                ...
                <span className="text-right" style={{ color: "#3b82f6" }}>{driverB.lastName}</span>
              </div>
              {[
                { label: "Posición", a: driverA.position + "°", b: driverB.position + "°", aWins: driverA.position < driverB.position },
                { label: "Puntos", a: driverA.points, b: driverB.points, aWins: driverA.points > driverB.points },
                { label: "Victorias", a: driverA.wins, b: driverB.wins, aWins: driverA.wins > driverB.wins },
                { label: "Equipo", a: teamMetaA.shortName, b: teamMetaB.shortName, aWins: null },
                { label: "Número", a: `#${driverA.number || "—"}`, b: `#${driverB.number || "—"}`, aWins: null },
                { label: "Nacionalidad", a: getNationalityFlag(driverA.nationality) + " " + (driverA.nationality || "—"), b: getNationalityFlag(driverB.nationality) + " " + (driverB.nationality || "—"), aWins: null },
              ].map(({ label, a, b, aWins }) => (
                <div key={label} className="grid grid-cols-3 items-center border-b border-[var(--color-border)] last:border-0 px-4 py-3">
                  <span className={`text-sm font-semibold tabular-nums ${aWins === true ? "text-[#e10600]" : "text-[var(--color-text-primary)]"}`}>
                    {a} {aWins === true && "✓"}
                  </span>
                  <span className="text-center text-xs text-[var(--color-text-muted)]">{label}</span>
                  <span className={`text-right text-sm font-semibold tabular-nums ${aWins === false ? "text-[#3b82f6]" : "text-[var(--color-text-primary)]"}`}>
                    {aWins === false && "✓ "}{b}
                  </span>
                </div>
              ))}
          ...
          <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartData} margin={{ top: 0, right: 8, left: -10, bottom: 0 }}>
                ...
                <XAxis dataKey="stat" tick={{ fill: "var(--color-text-muted)", fontSize: 11 }} axisLine={false} tickLine={false} />
                ...
                <Bar dataKey={driverA.lastName} fill="#e10600" radius={[4, 4, 0, 0]} />
                <Bar dataKey={driverB.lastName} fill="#3b82f6" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
...
function DriverSelect({
  ...
  return (
    <div>
      <label className="block text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: accentColor }}>
        {label}
      </label>
      <select
        value={value}
...
function DriverCompareCard({ driver, accent }: { driver: ProcessedDriver; accent: string }) {
  const teamMeta = getTeamMeta(driver.teamId);
  const flag = getNationalityFlag(driver.nationality);

  return (
    <div className="rounded-[var(--radius-lg)] border bg-[var(--color-surface)] overflow-hidden" style={{ borderColor: `${accent}40` }}>
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-[var(--color-text-muted)]">{driver.firstName}</p>
        <p className="text-xl font-bold text-[var(--color-text-primary)] mb-2">{driver.lastName}</p>
        <p className="text-xs font-medium mb-3" style={{ color: teamMeta.primaryColor }}>{teamMeta.shortName}</p>
        <div className="flex gap-2 text-xs text-[var(--color-text-muted)]">
          <span>{flag} {driver.nationality}</span>
