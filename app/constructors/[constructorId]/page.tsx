...
export default async function ConstructorDetailPage({ params, searchParams }: ConstructorDetailPageProps) {
  ...
  const [constructor, standing, driverStandings, currentResults, previousResults] = await Promise.all([
        ...
        <div className="p-6 sm:p-8">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex-1">
              ...
              <p className="mt-2 text-sm text-[var(--color-text-secondary)]">{teamMeta.name}</p>
              {standing && (
                <Badge
                  variant="team"
            ...
            {standing && (
              <div className="flex gap-6 sm:flex-col sm:gap-4 sm:text-right">
                <StatBox label="Posición" value={`${standing.position}°`} highlight color={teamMeta.primaryColor} />
            ...
            {teamDrivers.map((s) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[var(--color-text-muted)]">{s.code}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs text-[var(--color-text-muted)]">Puntos</p>
                  <p className="text-lg font-bold text-[var(--color-text-primary)] tabular-nums">{s.points}</p>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[400px]">
              <div className="grid grid-cols-[2rem_1fr_auto_auto_auto] border-b border-[var(--color-border)] bg-[var(--color-surface-raised)]/50 px-4 py-2.5 gap-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">R</span>
                <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">Carrera</span>
                <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">Piloto 1</span>
                <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">Piloto 2</span>
              ...
              {currentResults.map((race) => {
                const totalPoints = race.results.reduce((sum, r) => sum + r.points, 0);
                ...
                return (
                  ...
                  >
                    <span className="text-xs text-[var(--color-text-muted)] tabular-nums">{race.round}</span>
                    <span className="text-sm text-[var(--color-text-primary)] truncate">{race.raceName}</span>
                    <span className="text-xs text-[var(--color-text-secondary)]">
                      {driver1 ? `P${driver1.positionText} ${driver1.driverName.split(' ').pop()}` : "—"}
                    </span>
                    <span className="text-xs text-[var(--color-text-secondary)]">
                      {driver2 ? `P${driver2.positionText} ${driver2.driverName.split(' ').pop()}` : "—"}
                    </span>
...
function StatBox({ label, value, highlight = false, color }: { label: string; value: string | number; highlight?: boolean; color?: string }) {
  return (
    <div>
