  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                {driver.permanentNumber && (
                  <span className="text-5xl font-black tabular-nums leading-none" style={{ color: teamMeta.primaryColor }}>
                    #{driver.permanentNumber}
                  </span>
                )}
                {driver.code && (
              ...
              <div className="mb-4">
                <p className="text-lg text-[var(--color-text-secondary)]">{driver.givenName}</p>
            ...
            {standing && (
              <div className="flex gap-6 sm:flex-col sm:gap-4 sm:text-right">
                <StatBox label="Posición" value={`${standing.position}°`} highlight />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        <InfoCard label="Nacionalidad" value={`${flag} ${driver.nationality ?? "—"}`} />
        <InfoCard label="Fecha de nacimiento" value={driver.dateOfBirth ? formatRaceDate(driver.dateOfBirth) : "—"} />
        <InfoCard label="Temporada" value={season} />
        <InfoCard label="Equipo" value={teamMeta.shortName} color={teamMeta.primaryColor} />
      </div>
      ...
      {hasChart && (
        <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 mb-8">
          ...
          <p className="text-sm text-[var(--color-text-secondary)] mb-5">
            Puntos acumulados ronda a ronda — comparativa {prevYear} vs {currentYear}
          </p>
              ...
              {currentResults.map((race) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-sm font-bold tabular-nums ${isTop3 ? "text-[var(--color-f1-red)]" : "text-[var(--color-text-secondary)]"}`}>
                      {res?.positionText ?? "—"}
                    </span>
...
function StatBox({ label, value, highlight = false }: { label: string; value: string | number; highlight?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-3xl font-black tabular-nums mt-0.5 ${highlight ? "text-[var(--color-f1-red)]" : "text-[var(--color-text-primary)]"}`}>
        {value}
      </p>
    </div>
  );
...
function InfoCard({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3">
      ...
      <p className="text-sm font-medium" style={{ color: color ?? "var(--color-text-primary)" }}>{value}</p>
    </div>
  );
}
