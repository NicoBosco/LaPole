...
export function DriversPageClient({
  ...
  const { page, totalPages, paginated, next, prev, reset } = usePagination(
    filtered,
    PAGE_SIZE,
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div
        className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 animate-fade-in-up"
        ...
        <>
          ...
          {!query && activeTab === "all" && (
            <p className="mt-4 text-center text-sm text-[var(--color-text-muted)]">
              Mostrando {paginated.length} de {standings.length} pilotos en{" "}
              {season}
            </p>
          )}
        </>
