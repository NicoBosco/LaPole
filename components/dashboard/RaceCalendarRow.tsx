    ...
    >
      ...
      <div className="shrink-0 text-center w-8">
        <span className={cn(
          ...
          isNext ? "text-[var(--color-f1-red)]" : "text-[var(--color-text-muted)]"
        )}>
          R{race.round}
        </span>
      </div>
      ...
      <div className="shrink-0 text-right">
        <p className={cn(
          "text-xs font-medium",
          isNext ? "text-[var(--color-f1-red)]" : "text-[var(--color-text-secondary)]"
        )}>
          {formatRaceDate(race.date)}
        </p>
        {isPast && (
          <p className="text-xs text-[var(--color-success)] font-medium">Ver resultados →</p>
        )}
        {isNext && (
          <p className="text-xs text-[var(--color-f1-red)] font-semibold">Próxima</p>
        )}
      </div>
    </div>
  );
