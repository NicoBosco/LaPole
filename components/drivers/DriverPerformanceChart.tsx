...
function buildData(
  currentResults: ProcessedRaceWithResults[],
  previousResults: ProcessedRaceWithResults[]
): Array<{ round: number; ptsPrev?: number; ptsCurr?: number; label: string }> {

  const map: Record<number, { ptsPrev?: number; ptsCurr?: number; label: string }> = {};

  let cumulPrev = 0;
  previousResults.forEach((r) => {
    const round = r.round;
    cumulPrev += r.results[0]?.points ?? 0;
  ...
  currentResults.forEach((r) => {
    const round = r.round;
    cumulCurr += r.results[0]?.points ?? 0;
...
export function DriverPerformanceChart({
  ...
  const data = buildData(currentResults, previousResults);
