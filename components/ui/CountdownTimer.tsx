...
function computeCountdown(targetDate: string | Date, targetTime?: string) {
  const target = typeof targetDate === "string"
    ? (targetTime ? new Date(`${targetDate}T${targetTime}`) : new Date(`${targetDate}T00:00:00Z`))
    : targetDate;
...
export function CountdownTimer({ targetDate, targetTime }: CountdownTimerProps) {
  ...
  const [time, setTime] = useState(() => computeCountdown(targetDate, targetTime));
  ...
  return (
    <div className="flex items-end gap-2">
      {time.days > 0 && (
        <CountUnit value={time.days} label={time.days === 1 ? "día" : "días"} accent />
      )}
